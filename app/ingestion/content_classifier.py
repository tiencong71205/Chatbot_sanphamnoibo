...
class ContentTypeClassifier:
    ...
    SECTION_MAP = [
        ...
        # Checked BEFORE the generic "lắp đặt" installation pattern below.
        # Confirmed real bug: "Chế độ lắp đặt" ("installation MODE") names
        # an in-app SETTING the user picks (e.g. "Treo trần/Gắn tường"),
        # not an instruction to physically install the device -- but it
        # contains the literal substring "lắp đặt", so without this more
        # specific pattern checked first, it matched the plain
        # installation pattern and got tagged content_type="installation"
        # -- identical to the genuine physical-installation chapter's own
        # chunks, making the two indistinguishable downstream even when
        # deliberately trying to prioritize real installation content for
        # a "hướng dẫn lắp đặt" query.
        (re.compile(r"chế độ lắp đặt", re.IGNORECASE), "configuration"),
