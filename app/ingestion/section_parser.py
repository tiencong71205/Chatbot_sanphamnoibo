...
LETTER_REGEX = re.compile(r"^\s*([A-ZÀ-Ỹ])(?:\.|\))\s+(.+)$", re.IGNORECASE)
...
class SectionParser:
    ...
    def is_heading(elem: Any) -> tuple[bool, str, int, str]:
        if elem.kind == "heading":
            ...
            if m_num:
                num, t = m_num.groups()
                return True, num, level, t.strip()
            m_letter = LETTER_REGEX.match(title)
            if m_letter:
                num, t = m_letter.groups()
                return True, num, level, t.strip()
        ...
        m_letter = LETTER_REGEX.match(text)
        if m_letter and text.upper() == text:
            num, t = m_letter.groups()
            return True, num, 1, t.strip()
        ...
        if m_num:
            num, t = m_num.groups()
            # Mis-styled template headings use List Paragraph. Under a
            # lettered Heading 1, ``1.`` is level 2 and ``1.1.`` is level 3.
            lvl = len(num.split('.')) + 1
