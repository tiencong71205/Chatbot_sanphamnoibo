...
class ChatbotService:
    ...
    _COMPARE_FOCUS = (
        "mã sản phẩm product_id model mục đích thông số kỹ thuật "
        "nguồn cấp điện áp truyền thông giao tiếp "
        "công suất đầu ra nút bấm điều khiển tại chỗ điều khiển từ xa "
        "điều khiển giọng nói hẹn giờ kích thước môi trường đèn báo "
        "tính năng chính"
    )
    _COMPARE_FIELD_TERMS = {
        # These are deliberately separate.  The internal retrieval key stored
        # in Qdrant (for example ``cong_tac_dimmer``) is not a business
        # product_id, and a commercial model must never substitute for either.
        "business_product_id": ("mã sản phẩm", "product_id"),
        "model": ("model", "mã model"),
        "purpose": ("mục đích", "tóm tắt sản phẩm", "mô tả giải pháp"),
        ...
        "indicator": (
            "đèn báo", "đèn nền", "led", "màu cam", "màu xanh",
            "màu đỏ", "còi báo",
        ),
        "dimensions": ("kích thước", "mm"),
        "environment": ("nhiệt độ", "độ ẩm", "môi trường"),
        "feature": (
    ...
    def _retrieve_named_products(
        ...
        for product in products:
            ...
            # Identity in Qdrant payload is an internal retrieval key.  Add a
            # small trusted identity block from the catalog so the LLM receives
            # the business product_id and verified model as two distinct
            # fields, without requiring Qdrant/BM25 re-ingestion.
            if comparison:
                identity_result = self._comparison_identity_result(
                    product_id,
                    product_name,
                )
                if identity_result:
                    product_results = self._deduplicate_results(
                        [identity_result] + product_results
                    )
    ...
    def _comparison_identity_result(
        self,
        retrieval_product_key: str,
        fallback_product_name: str,
    ) -> Optional[Dict[str, Any]]:
        """Build one trusted comparison identity chunk from the catalog.

        ``retrieval_product_key`` is the slug used to filter Qdrant.  It is
        intentionally labelled as internal metadata and never exposed as the
        business ``product_id`` value in the evidence content.
        """
        catalog_path = Path(self.settings.product_catalog_path)
        if not catalog_path.exists():
            for alternative in (
        ...
        try:
            ...
            logger.exception("Cannot read product identity catalog: %s", catalog_path)
            return None

        item = next(
            (
                row for row in catalog
                if str(row.get("product_id", "")) == retrieval_product_key
            ),
            None,
        )
        if not item:
        ...
        business_product_id = str(
            item.get("business_product_id", "")
        ).strip()
        model = str(item.get("model", "")).strip()
        if not business_product_id:
            business_product_id = "Chưa có dữ liệu"
        if not model or model.lower() == "chưa xác định - cần xác nhận":
            model = "Chưa có dữ liệu"

        product_name = str(
            item.get("product_name") or fallback_product_name
        ).strip()
        return {
            "payload": {
                "chunk_id": f"{retrieval_product_key}_trusted_identity",
                # Keep the existing payload key for grouping/filtering only.
                "product_id": retrieval_product_key,
                "retrieval_product_key": retrieval_product_key,
                "product_name": product_name,
                "product_group": str(item.get("product_group", "")),
                "source_document": str(item.get("source_file", "")),
                "source_section": "Nhận diện sản phẩm",
                "content_type": "identity",
                "content": (
                    f"Tên sản phẩm: {product_name}\n"
                    f"Mã sản phẩm / product_id: {business_product_id}\n"
                    f"Model: {model}\n"
                    "Ghi chú: retrieval_product_key là khóa nội bộ để truy xuất, "
                    "không phải mã sản phẩm và không được hiển thị trong bảng."
                ),
            },
            "rrf_score": 1.0,
        }
    ...
    def _select_comparison_evidence(
        ...
        # Identity, technical tables and overview are independent evidence:
        # business product_id/model, core specifications, then purpose/features.
        for required_type in ("identity", "specification", "overview"):
            candidates = [
                item for item in results
        ...
        # The sixth chunk is exceptional: reserve it for an explicitly
        # exhaustive request.  A generic comparison stays in the normal 3-5
        # range even when the source document contains many optional fields.
        exhaustive = any(
            term in question.lower()
            for term in ("toàn bộ", "đầy đủ", "chi tiết tất cả")
        )
        if exhaustive and len(selected) < cls._COMPARE_MAX_CHUNKS_PER_PRODUCT:
            missing = requested_fields - covered()
            candidates = [
                item for item in ranked
                if str(item.get("payload", {}).get("chunk_id") or id(item))
