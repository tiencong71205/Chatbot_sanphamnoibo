...
class BM25Retriever:
    ...
    def retrieve(
        ...
        for r in raw_results:
            payload = r.get("chunk", {})
            if pid:
                # Exact match only. A substring/containment check here was a
                # confirmed bug: many real product_ids are substrings of one
                # another by construction (e.g. "cong_tac_thong_minh" is a
                # substring of "cong_tac_thong_minh_dimmer"), so asking about
                # one product could silently pull in another's chunks even
                # though Qdrant's own filter (exact MatchValue) correctly
                # rejected them — the two retrievers disagreed, and RRF
                # fusion then merged in the wrongly-admitted BM25 hits.
