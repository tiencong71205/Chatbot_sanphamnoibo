...
class BM25Store:
    ...
    def build_index(self, chunks: List[Chunk]) -> None:
        ...
        # Index title + heading_path alongside body content -- confirmed
        # real bug indexing content alone: a short, keyword-dense chunk
        # (an app-config step whose body happens to restate the literal
        # query terms via the generic navigation preamble every
        # "Hướng dẫn" step shares -- "Tại màn hình Trang chủ, chọn xem
        # chi tiết [Tên sản phẩm]") out-ranked the genuinely correct
        # physical-installation chunk, whose body never restates the
        # product name and lives under a heading_path ("Hướng dẫn lắp
        # đặt > Tiến hành lắp đặt") that closely echoes the user's actual
        # query wording -- but that echo was completely invisible to
        # BM25, since only body content was ever indexed. Verified with a
        # 17-chunk simulated corpus matching this product's real scale:
        # the correct chunk went from absent in the top 5 to outranking
        # the wrong one after adding title/heading_path to the index.
        self._corpus = [
            tokenize_vi(f"{c.title} {c.heading_path} {c.content}") for c in chunks
        ]
