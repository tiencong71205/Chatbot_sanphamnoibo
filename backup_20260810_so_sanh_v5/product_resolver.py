...
class ResolvedProduct:
    """Backward-compatible typed result used by metadata filtering/tests."""
...
class ProductResolver:
    ...
    def resolve_all(self, query: str) -> List[Dict[str, Any]]:
        ...
        for item in self.catalog:
            ...
            if (
                model
                and model != "chưa xác định - cần xác nhận"
                and model in q_norm
            ):
...
def resolve_product(
    ...
    """Backward-compatible functional resolver API."""
    if product_id_hint:
    ...
    match = ProductResolver(catalog_path).resolve(query)
    return ResolvedProduct(
        product_id=match.get("product_id") or None,
