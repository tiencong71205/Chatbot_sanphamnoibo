...
def test_mesh_sensor_is_present_in_unified_catalog():
    resolver = ProductResolver(CATALOG_PATH)
    mesh = next(
        item
        for item in resolver.catalog
        if item.get("product_id") == "cam_bien_cua_mesh"
    )
    assert mesh["product_name"] == "Cảm biến cửa Mesh"
    assert mesh["source_file"] == "02_Cam_bien_cua_Mesh_v1.0.docx"
