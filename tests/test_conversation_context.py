...
def test_uncited_sources_are_not_returned_to_ui():
    """A genuine 'no data in the document' answer must not have unrelated
    sources attached to it — that would misleadingly imply those sources
    back up an answer that explicitly says nothing was found."""
    sources = [
        SourceReference(source_id="SOURCE_1"),
        SourceReference(source_id="SOURCE_2"),
    ]
    ...
    assert (
        filter_cited_sources(
            "Tài liệu hiện có chưa cung cấp thông tin này; cần xác nhận với bộ phận kỹ thuật Vhomenex.",
            sources,
        )
        == []
    )
...
def test_substantive_answer_missing_citation_footer_falls_back_to_all_sources():
    """A real, substantive answer that omits the '📚 Nguồn tham khảo:' footer
    (model forgot the instruction, or generation was cut off by max_tokens)
    must not silently lose every source — that looks unverifiable to the
    user even though real context was used to write the answer. Falling
    back to the full source set is safer than showing none."""
    sources = [
        SourceReference(source_id="SOURCE_1"),
        SourceReference(source_id="SOURCE_2"),
    ]

    result = filter_cited_sources(
        "Điện áp hoạt động của thiết bị là 220V, công suất tối đa 1000W.",
        sources,
    )
    assert [s.source_id for s in result] == ["SOURCE_1", "SOURCE_2"]


def test_no_sources_available_returns_empty_regardless_of_answer_text():
    assert filter_cited_sources("Bất kỳ câu trả lời nào.", []) == []


def test_connection_target_is_not_treated_as_second_primary_product():
    resolver = ProductResolver(CATALOG_PATH)

    primary = resolver.resolve_primary(
        "Hướng dẫn kết nối Cảm biến cửa Mesh với bộ điều khiển trung tâm Gateway."
    )

    assert primary["product_id"] == "cam_bien_cua_mesh"
    assert primary["product_name"] == "Cảm biến cửa Mesh"
    assert primary["match_type"] == "exact_name"


def test_gateway_procedure_keeps_b1_to_b8_as_separate_steps():
    selected = [
        {
            "payload": {
                "chunk_type": "procedure",
                "feature_name": "Chế độ kết nối với bộ điều khiển trung tâm",
                "content": "\n\n".join(
                    f"B{index}: Nội dung bước {index}"
    ...
    answer = ChatbotService._finalize_answer(
        "Hướng dẫn kết nối Cảm biến cửa Mesh với Gateway.",
        "1. Bước gộp sai\n\n-",
        selected,
    )

    for index in range(1, 9):
        assert answer.count(f"**B{index}:**") == 1
    assert "SOURCE_1" in answer
    assert not answer.rstrip().endswith("-")


def test_usb_converter_procedure_keeps_b1_to_b7():
    selected = [
        {
            "payload": {
                "chunk_type": "procedure",
                "feature_name": "Chế độ kết nối với thiết bị Vconnex USB Converter",
                "content": "\n\n".join(
                    f"B{index}: Nội dung USB bước {index}"
    ...
    answer = ChatbotService._finalize_answer(
        "Hướng dẫn kết nối bằng Vconnex USB Converter.",
        "Câu trả lời bị gộp bước.",
        selected,
    )

    assert all(f"**B{index}:**" in answer for index in range(1, 8))
    assert "**B8:**" not in answer
...
def test_chat_service_applies_history_product_filter():
    ...
    assert retriever.last_call["product_match_type"] == "inferred"
...
def test_compare_retrieves_each_named_product_with_hard_filter():
    ...
    assert response.debug_info["total_ms"] >= 0
